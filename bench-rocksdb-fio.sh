...
for ((i=1; i<=10; i++))
do
  echo "------------ rocksdb fio  with out ioi case$i benchmark --------------------"
  ...
  kubectl  get po -A |grep "sysbench-job" | awk '{print $2}'| xargs -n1 kubectl logs |tail -n 30 >  ./result/bench-rocksdb-1-ioi-nn-result-10$i.log
